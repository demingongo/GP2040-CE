...
void EspUartBridgeAddon::process() {
    ...
    // Skip transmission while the USB host has the device mounted (wired mode active).
    // tud_mounted() is more reliable than gpio_get(PICO_VBUS_PIN) on RP2350 and is
    // semantically correct: suppress UART exactly when the Pico2 is enumerated as a
    // USB HID gamepad. Reading this flag from Core 1 is safe — it is a single-byte
    // value written atomically by Core 0.
    if (disableWhenUsbConnected && tud_mounted()) {
        return;
    }
